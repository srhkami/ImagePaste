...
def add_table(doc, options, no, img_path, show_type='H', mode=1):
  '''
  :param doc: 傳入的doc文件
  :param options: 讀取的預設值
  :param no: 序號
  :param img_path: 單一圖片路徑；如果是2，則為清單
  ...
  try:
    ...
    # 寫入文字及對齊
    table.cell(1, 2).text = number(no)  ## 寫入編號
    ...
    table.cell(3, 1).text = options['describe']
    table.cell(3, 1).vertical_alignment = WD_ALIGN_VERTICAL.CENTER

    log().debug(f'編號{no} 表格創建成功')
  except:
    log().error(f'編號{no} 表格創建失敗', exc_info=True)

  # 寫入圖片
  try:
    if mode == 1:
      if show_type == 'H':
        table.cell(0, 0).paragraphs[0].add_run().add_picture(img_path, height=Cm(9))
      else:
        table.cell(0, 0).paragraphs[0].add_run().add_picture(img_path, width=Cm(15))
    if mode == 2:
    ...
    log().debug(f'編號{no} 圖片黏貼完成')
  except:
    log().error(f'編號{no} 圖片黏貼失敗', exc_info=True)
